...
int main() {
    ...
    while (running) {
        ...
        try {
            choice = std::stoi(input_line);
        } catch (const std::invalid_argument& ia) {
            choice = -1;  // Invalid input, will go to default case
        } catch (const std::out_of_range& oor) {
            choice = -1;  // Input out of range, will go to default case
        }
        ...
        switch (choice) {
            ...
            case 2:  // Enter puzzle manually
                ...
                if (!solver.isValidPuzzle()) {
                    std::cout
                        << "Warning: The entered puzzle may not be valid.\n";
                }
